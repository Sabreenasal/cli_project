...
def remove_garment
  puts "\n"
  if @outfits.empty?
    puts "You currently do not have any outfits in your closet"
  else
    puts "What is the oufit you want to wear today? Example: blue top"
    article_of_clothing = gets.chomp
    closet_search = @outfits.find { |outfit|
      if outfit.to_s.downcase == article_of_clothing
        @outfits.delete(outfit)
        puts "You took out: #{article_of_clothing}! You look amazing"
      elsif outfit != article_of_clothing
